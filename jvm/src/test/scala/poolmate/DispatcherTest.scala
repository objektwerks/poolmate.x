...
final class DispatcherTest extends AnyFunSuite with Matchers with LazyLogging:
  ...
  def testDispatcher(dispatcher: Dispatcher, store: Store): Unit =
    ...
    var surface = Surface(poolId = pool.id, installed = 20010101, kind = "concrete", cost = 2400)
    ...
    var deck = Deck(poolId = pool.id, installed = 20010103, kind = "tile", cost = 1200)
    deck = testAddDeck(dispatcher, pool, deck)
    testListDecks(dispatcher, pool)
    testUpdateDeck(dispatcher, pool, deck.copy(kind = "pavers"))

    var pump = Pump(poolId = pool.id, installed = 20010101, model = "hayward", cost = 450)
